...
async function testElasticsearch() {
  ...
  try {
    ...
    console.log(
      `Found ${searchData.total} boats:`,
      searchData.boats.map((b) => b.name)
    );
    console.log();
    ...
    console.log(
      `Found ${searchData.total} Motor boats:`,
      searchData.boats.map((b) => b.name)
    );
    console.log();
    ...
    searchResponse = await fetch(
      `${BASE_URL}/boats/search?yearMin=2020&yearMax=2022`
    );
    searchData = await searchResponse.json();
    console.log(
      `Found ${searchData.total} boats (2020-2022):`,
      searchData.boats.map((b) => `${b.name} (${b.year})`)
    );
    console.log();
    ...
    searchResponse = await fetch(
      `${BASE_URL}/boats/search?q=wind&type=Sailing`
    );
    searchData = await searchResponse.json();
    console.log(
      `Found ${searchData.total} boats:`,
    ...
    allBoats.forEach((boat) =>
      console.log(`  - ${boat.name} (${boat.type}, ${boat.year})`)
    );
